...
% Wymnażanie przez okno Blackmana aby zredukować efekt przecieków
% częstotliwościowych
w  = blackman(N); 
w  = w';            
hw = h.*w;                   
...
for k=1:NF-1
    H(k)  =sum (h  .* exp(-j*2*pi*fn(k)*m));%obliczanie charakterystyki częstotliwościowej filtra
...
%% odtworzone próbki sygnału
%z powodu małej liczby składowej  i braku znajomości fazy odwzorowanie jest
%mało precyzyjne i przesunięte w fazie 

t=(0:length(x2)-1)/fs;%oś czasu dopasowana do x2

m_rec=A1*cos(2*pi*f1*fs*t)+A2*cos(2*pi*f2*fs*t)+A3*cos(2*pi*f3*fs*t);
phi=pi/2;

x_rec=(1+m_rec).*cos(2*pi*fc*t+phi);

figure;
plot(t(1:200),x2(1:200),'b');
hold on
plot(t(1:200),x_rec(1:200),'r');
title('Porównanie orginalnego i odtworzonego sygnału');
xlabel('Czas');
ylabel('Amplituda');
grid on;
