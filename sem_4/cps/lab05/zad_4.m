...
grid; xlabel('w [rad*Hz]'); 
title('Analog Proto |H(f)'); 
figure; 
plot(real(z),imag(z),'ro',real(p),imag(p),'b*',c,s,'k-'); 
grid; 
title('Analog Proto ZP');

% Transformacja czestotliwosci: znormalizowany (w0=1) LowPass --> xxxPass, xxxStop
% Funkcje xx2yy z Signal Processing Toolbox
[b,a] = lp2lp(b,a,2*pi*f0); % LowPass na LowPass przesuwa granicę pasma (np. z ω = 1 → 2π·64kHz),
% [b,a] = lp2hp(b,a,2*pi*f0); % LowPass na HighPass  zamienia zbocze dolnoprzepustowe w zbocze wysokoprzepustowe,
% [b,a] = lp2bp(b,a,2*pi*sqrt(f1*f2),2*pi*(f2-f1)); % LowPass na BandPass dolne i górne odcięcie (typowe dla filtrów pasmowych),
% [b,a] = lp2bs(b,a,2*pi*sqrt(f1*f2),2*pi*(f2-f1)); % LowPass na BandStop robi wycięcie pasma (filtr zaporowy).
z=roots(b); p=roots(a);
...
grid; 
figure; 
impulse(b,a); % odpowiedz filtra na pobudzenie impulsowe (z Control Toolbox)
