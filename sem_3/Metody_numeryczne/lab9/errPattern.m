...
% idzie nie tak :)
figure;
x=xor(dataIn,dataRaw);
bit_error1=find(x(1:end),length(x));
result1=diff(bit_error1);
histogram(result1)

y=xor(convDataIn,convDataOut);
bit_error2=find(y(1:end),length(y));
result2=diff(bit_error2);
figure;
histogram(result2);
