...
numStr1=randi([0 M-1],1,numSyms);
numStr2=randi([0 M-1],1, numSyms);

%TODO 2: przekształć wejściowe strumienie danych na ciąg symboli QPSK, użyj funkcji qammod

symStr1=qammod(numStr2,M); %wygeneruj pierwszy sygnał M-QAM
...
symStr1=upfirdn(symStr1,rrcFlt,sps,1); %zmiana w TODO6% #2
symStr2=upfirdn(symStr2,rrcFlt,sps,1); %zmiana w TODO6% #2
...
detSig=fshift(compositeSig,-f1,Ts);
...
detSyms = upfirdn(detSig, rrcFlt, 1, sps); % zmiana w TODO6% #3
detSyms = detSyms(floor(length(rrcFlt)/2)+1:end-floor(length(rrcFlt)/2));
