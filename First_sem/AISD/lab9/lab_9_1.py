...
def check_fit(x,y,knapsack):
    ...
    for x_start in  range(len(knapsack)-x+1): #other rotation
        for y_start in range(len(knapsack)-y+1):
            fits=True
            for x_iter in range(x):
                for y_iter in range(y):
...
def main():
    ...
    for size in ['20','100','500','1000']:
        start=time.time()
        with open('C:\\Users\\mateu\\Desktop\\PI\\packages'+size +'.txt') as txt:
        ...
        for item in items:
            ...
            if fit:
                ...
                if(fit[2]):
                    ...
                    for x in range(fit[0],item[2]+fit[1]):
                        for y in range(fit[1],item[2]+fit[1]):
                            knapsack[x][y]=iter

                value+=item[3]
    print(knapsack)
    print(("zachłanny"+str(size))+','+str(time.time()-start))
    print(value)
    sum =0
    for item in items:
        sum+=item[3]
